...
def main():
	...
	for arg in sys.argv[1:]:
		if arg in ['-?', '-h', '--help', '-help']:
			...
			sys.stdout.flush()
			sys.exit()
		m = re.search(r'^(?:CC|CXX)=(.*)$', arg)
		if m:
			cmake_args.append('-DCMAKE_CXX_COMPILER='+m.group(1))
		else:
			m = re.search(r'^(?:C|CXX)FLAGS=(.*)$', arg)
			if m:
				cmake_args.append('-DCMAKE_CXX_FLAGS='+m.group(1))
			else:
	...
	make_status = subprocess.call(['time', '-f', (ansi_magenta+'`make` took %E'+ansi_end), 'make'] + make_flags)
	...
	if make_status != 0:
		subprocess.call(['make']) #to get just one file's error messages not mixed up with the others
